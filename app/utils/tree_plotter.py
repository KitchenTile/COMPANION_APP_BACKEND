...
#BAD PATHS
def get_gpt_alternate_edges(data):
...
#CORRECTIONS
def get_gpt_actions_for_failures(failure_nodes, tools):
    potential_actions = []
...
#PREVENTIONS
def get_gpt_failure_preventions(right_path, failure_paths):

    system_prompt="""
    Given a right path and failure nodes for each node in the right path, provide preventive meassures for user's to take to prevent them going into the failure paths. 
    
    RULES:
    1. For each correct path and its respective failure node, generate a single intervention
     """
    ...
    response = client.responses.parse(
        ...
        instructions=system_prompt,
        input = [
            {"role": "user", "content": f"This is the correct path: {right_path}.These are the failure nodes: {failure_paths}"}
        ],
        text_format=Preventions
    )    
